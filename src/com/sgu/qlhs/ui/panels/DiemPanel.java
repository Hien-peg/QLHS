...
import com.sgu.qlhs.ui.model.DiemTableModel;
...
public class DiemPanel extends JPanel {
    public DiemPanel(){ setLayout(new BorderLayout()); setOpaque(false);
        var outer = new RoundedPanel(18, CARD_BG, CARD_BORDER); outer.setLayout(new BorderLayout());
        var lbl = new JLabel("Điểm"); lbl.setBorder(new EmptyBorder(12,16,8,16)); lbl.setFont(lbl.getFont().deriveFont(Font.BOLD,18f));
        outer.add(lbl, BorderLayout.NORTH);
        var table = new JTable(new DiemTableModel()); table.setAutoCreateRowSorter(true);

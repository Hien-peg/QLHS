    ...
    (
        ...
        FROM
            (
                ...
                CROSS JOIN (SELECT 1 AS HocKy UNION SELECT 2 AS HocKy) hk
                CROSS JOIN NienKhoa nk
                WHERE nk.MaNK = 1 -- Chỉ thêm cho niên khóa 1 (2024-2025)
            ) AS all_combos
        -- 2. Loại bỏ những kết hợp đã tồn tại trong bảng Diem
        LEFT JOIN
            Diem d ON all_combos.MaHS = d.MaHS
    ...
    (
        ...
        FROM
            (
                ...
                CROSS JOIN (SELECT MaMon FROM MonHoc WHERE LoaiMon = 'TinhDiem') mh
                CROSS JOIN (SELECT 1 AS HocKy UNION SELECT 2 AS HocKy) hk
                CROSS JOIN NienKhoa nk
                WHERE nk.MaNK = 1
            ) AS all_combos
    ...
    (
        ...
        FROM
            (
                ...
                CROSS JOIN (SELECT MaMon FROM MonHoc WHERE LoaiMon = 'DanhGia') mh
                CROSS JOIN (SELECT 1 AS HocKy UNION SELECT 2 AS HocKy) hk
                CROSS JOIN NienKhoa nk
                WHERE nk.MaNK = 1
            ) AS all_combos
